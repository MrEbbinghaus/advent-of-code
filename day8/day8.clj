...
(defn advent-lang [state [target operation op-param _ condition cond_op cond_value]]
    (let [mappings {"dec" '-
                    "inc" '+
                    "!=" 'not=}]
      (if (eval (list (get mappings cond_op (read-string cond_op)) (get state condition 0) (read-string cond_value)))
         (update state target (fnil #(eval (list (get mappings operation (read-string operation)) % (read-string op-param))) 0))
         state)))

(defn task1 [input]
    (->> (reduce advent-lang {} input)
        (vals)
        (sort >)
        (first)))
...
(defn task2 [input]
    (:biggest
        (reduce (fn [state statement]
                  (let [result (advent-lang state statement)
                        biggest (first (sort > (vals result)))]
                    (update result :biggest #(if (< % biggest) biggest %))))
