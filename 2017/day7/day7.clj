...
(str/split "wdugfj (72519) -> qdqkaw, xfcoi, rtydk, kbbgkmr, uxsrdpn" #"(\) -> )|\(?,?\s\(?")
(str/split "xizhxs (18)" #"( -> )|\s")

(defn parse-entry [entry]
    (let [[name weight & children] (str/split entry #"\s\(|(\) -> )|,\s|\)")]
      (merge
          {:name name
...
(defn vector-tree->map-tree [parsed]
    (into (hash-map) (map #(vector (:name %) (dissoc % :name)) parsed)))

(defn task1 [input]
    (let [parsed (map parse-entry input)
          all-children (apply union (map :children parsed))]
      (:name (first (filter #(not (contains? all-children (:name %))) parsed)))))
...
(defn real-weight [tree node]
    (if-let [children (get-in tree [node :children])]
      (reduce + (get-in tree [node :weight]) (map (partial real-weight tree) children))
      (get-in tree [node :weight])))

(defn balanced? [tree node]
    (let [children (map (partial real-weight tree) (get-in tree [node :children]))]
      (if (empty? children)
        true
        (apply = children))))

(defn black-sheep
    "Returns the name of the only node which is unbalanced."
    [tree node]
    (let [children (get-in tree [node :children])]
        (second (flatten (filter #(= 1 (count (second %))) (group-by #(real-weight tree %) children)))))) ; meh

(defn task2 [input]
  (let [tree (vector-tree->map-tree (map parse-entry input))
        root (task1 input)]
    (loop [father root]
      (let [bad-son (black-sheep tree father)
            should-be (real-weight tree (first (disj (get-in tree [father :children]) bad-son)))]
        (if (balanced? tree bad-son)
          (+ (get-in tree [bad-son :weight]) (- should-be (real-weight tree bad-son)))
          (recur bad-son))))))
