(def my-input [[1224  926  1380  688  845  109  118  88  1275  1306  91  796  102  1361  27  995]
...
(defn- evenly-divisible-quotient [num rest]
    (let [evenly-divisible (filter #(= 0 (mod num %)) rest)]
        (if (< 0 (count evenly-divisible))
            (/ num (first evenly-divisible))
            0)))


(defn even-dividers [row]
    (let [s (reverse (sort row))]
        (reduce + (map-indexed (fn [i v] (evenly-divisible-quotient v (drop (inc i) s))) s))))

(defn process [input f]
    (reduce + (map f input)))

(process my-input difference)

(process my-input even-dividers)

(even-dividers [5 9 2 8])
